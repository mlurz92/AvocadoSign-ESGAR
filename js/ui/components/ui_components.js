window.uiComponents = (() => {
    ...
    function createT2CriteriaControls(initialCriteria, initialLogic) {
        ...
        const isEsgarLogic = initialLogic === 'KOMBINIERT';
        ...
        const createButtonOptions = (key, isChecked, criterionLabel) => {
            ...
            return values.map(value => {
                ...
                return `<button class="btn t2-criteria-button criteria-icon-button ${isActiveValue ? 'active' : ''} ${(!isChecked || isEsgarLogic) ? 'inactive-option' : ''}" data-criterion="${key}" data-value="${value}" data-tippy-content="${buttonTooltip}" ${(!isChecked || isEsgarLogic) ? 'disabled' : ''}>${icon}</button>`;
            }).join('');
        };
            ...
            return `
                <div class="col-md-6 criteria-group">
                    <div class="form-check mb-2">
                        <input class="form-check-input criteria-checkbox" type="checkbox" value="${key}" id="check-${key}" ${isChecked ? 'checked' : ''} ${isEsgarLogic ? 'disabled' : ''}>
            ...
            <div class="card criteria-card" id="t2-criteria-card">
                <div class="card-header d-flex justify-content-between align-items-center">
                    ...
                    <div class="d-flex align-items-center">
                        ${isEsgarLogic ? `<span class="badge bg-info text-dark me-3"><i class="fas fa-lock me-1"></i> ESGAR Mode Active</span>` : ''}
                        <div class="form-check form-switch" data-tippy-content="${tooltips.t2Logic.description}">
                             ...
                             <input class="form-check-input" type="checkbox" role="switch" id="t2-logic-switch" ${logicChecked ? 'checked' : ''} ${isEsgarLogic ? 'disabled' : ''}>
                     ...
                     <div class="row g-4">
                        ${createCriteriaGroup('size', 'Size', 't2Size', (key, isChecked) => `
                            <div class="d-flex align-items-center flex-wrap">
                                 ...
                                 <input type="range" class="form-range criteria-range flex-grow-1 me-2" id="range-size" min="${min}" max="${max}" step="${step}" value="${formattedThresholdForInput}" ${(!isChecked || isEsgarLogic) ? 'disabled' : ''} data-tippy-content="Set short-axis diameter threshold (≥).">
                                 ...
                                 <input type="number" class="form-control form-control-sm criteria-input-manual" id="input-size" min="${min}" max="${max}" step="${step}" value="${formattedThresholdForInput}" ${(!isChecked || isEsgarLogic) ? 'disabled' : ''} style="width: 70px;" aria-label="Enter size manually" data-tippy-content="Enter threshold manually.">
                            </div>
                        `)}
    ...
    function createBruteForceModalContent(resultsData) {
        if (!resultsData || !resultsData.results || resultsData.results.length === 0) return '<p class="text-center text-muted">No brute-force results available.</p>';
        ...
        let html = `
            ...
            <div class="table-responsive">
                <table class="table table-sm table-striped small" id="bruteforce-results-table">
                    <thead><tr><th>Rank</th><th>${metric}</th><th>Logic</th><th>Criteria</th></tr></thead>
                    <tbody>`;
        ...
        for (let i = 0; i < results.length; i++) {
            ...
            html += `<tr><td>${currentRank}</td><td>${formatNumber(result.metricValue, 4, 'N/A', true)}</td><td>${result.logic.toUpperCase()}</td><td><code>${formatCriteriaFunc(result.criteria, result.logic)}</code></td></tr>`;
    ...
    function createBruteForceOverviewTableHTML(allBfResultsByCohort) {
        if (!allBfResultsByCohort || Object.keys(allBfResultsByCohort).length === 0) return '<p class="text-muted small p-3">No saved brute-force optimization results available. Run optimization on the Analysis tab.</p>';
        const formatCriteriaFunc = typeof window.studyT2CriteriaManager !== 'undefined' ? window.studyT2CriteriaManager.formatCriteriaForDisplay : (c, l) => 'N/A';
        const na = window.APP_CONFIG.NA_PLACEHOLDER;
        let tableHTML = `<div class="table-responsive"><table class="table table-sm table-striped small caption-top"><caption>Saved Brute-Force Optima</caption><thead><tr><th>Cohort</th><th>Target Metric</th><th>Best Value</th><th>Criteria</th><th>Actions</th></tr></thead><tbody>`;
                ...
                if (cohortMetrics.length > 0) {
                    ...
                    cohortMetrics.sort((a,b) => a.localeCompare(b)).forEach(metricName => {
                        ...
                        if(result && result.bestResult) {
                            tableHTML += `<tr><td></td><td>${metricName}</td><td>${formatNumber(result.bestResult.metricValue, 4, na, true)}</td><td><code>${formatCriteriaFunc(result.bestResult.criteria, result.bestResult.logic, true)}</code></td><td><button class="btn btn-sm btn-outline-primary p-0 px-2" data-action="apply-saved-bf" data-cohort="${cohortId}" data-metric="${metricName}" data-tippy-content="Apply this set of criteria to the 'Define T2 Criteria' panel.">Apply</button></td></tr>`;
                        }
                    });
                }
            }
        });
        if (!hasContent) return '<p class="text-muted small p-3">No saved brute-force optimization results available. Run optimization on the Analysis tab.</p>';
    ...
    function createBruteForceRunnerCardHTML(state, payload, bfWorkerAvailable, currentCohort, selectedMetric) {
        ...
        let startButtonDisabled = true, cancelButtonDisabled = true, applyBestButtonDisabled = true, showDetailsButtonDisabled = true, progressHTML = '';
        ...
        switch (state) {
            case 'initial':
                startButtonDisabled = !bfWorkerAvailable; cancelButtonDisabled = true; applyBestButtonDisabled = !currentBestResultForMetric; showDetailsButtonDisabled = !currentBestResultForMetric;
                progressHTML = `<p class="mb-2 small text-muted">Select metric and start optimization.</p><div class="progress" style="height: 5px;"><div class="progress-bar" role="progressbar" style="width: 0%;" aria-valuenow="0" aria-valuemin="0" aria-valuemax="100"></div></div><p class="small text-muted mt-2 mb-0">Progress: 0%</p>`;
                break;
            case 'progress':
                startButtonDisabled = true; cancelButtonDisabled = false; applyBestButtonDisabled = true; showDetailsButtonDisabled = true;
                const tested = payload?.tested || 0, total = payload?.total || 1, percent = total > 0 ? Math.floor((tested / total) * 100) : 0, currentBest = payload?.currentBest;
                ...
                progressHTML = `<p class="mb-2 small text-muted">Running on cohort: <strong>${getCohortDisplayName(currentCohort)}</strong>, Metric: <strong>${payload?.metric || selectedMetric}</strong></p><div class="progress" style="height: 5px;"><div class="progress-bar progress-bar-striped progress-bar-animated bg-primary" role="progressbar" style="width: ${percent}%;" aria-valuenow="${percent}" aria-valuemin="0" aria-valuemax="100"></div></div><p class="small text-muted mt-2 mb-0">Combinations tested: ${formatNumber(tested, 0)} / ${formatNumber(total, 0)} (${percent}%)</p><p class="small text-muted mb-0">${currentBestInfo}</p>`;
                break;
            case 'result':
                startButtonDisabled = !bfWorkerAvailable; cancelButtonDisabled = true; applyBestButtonDisabled = !(payload && payload.bestResult); showDetailsButtonDisabled = !(payload && payload.bestResult);
                const best = payload, durationSeconds = (best?.duration || 0) / 1000;
                progressHTML = `<p class="mb-2 small text-muted">Optimization finished for cohort: <strong>${getCohortDisplayName(best?.cohort || currentCohort)}</strong>, Metric: <strong>${best?.metric || selectedMetric}</strong></p><div class="progress" style="height: 5px;"><div class="progress-bar bg-success" role="progressbar" style="width: 100%;" aria-valuenow="100" aria-valuemin="0" aria-valuemax="100"></div></div><p class="small text-muted mt-2 mb-0">Total Tested: ${formatNumber(best?.totalTested || 0, 0)} in ${formatNumber(durationSeconds, 1)} seconds</p><p class="small text-muted mb-0">Best Result: <strong>${formatNumber(best?.bestResult?.metricValue, 4)}</strong> with <code>${formatCriteriaFunc(best?.bestResult?.criteria, best?.bestResult?.logic, true)}</code></p>`;
                break;
            case 'cancelled':
                startButtonDisabled = !bfWorkerAvailable; cancelButtonDisabled = true; applyBestButtonDisabled = !currentBestResultForMetric; showDetailsButtonDisabled = !currentBestResultForMetric;
                progressHTML = `<p class="mb-2 small text-muted">Optimization cancelled for cohort: <strong>${getCohortDisplayName(payload?.cohort || currentCohort)}</strong>.</p><div class="progress" style="height: 5px;"><div class="progress-bar bg-warning" role="progressbar" style="width: 0%;" aria-valuenow="0" aria-valuemin="0" aria-valuemax="100"></div></div><p class="small text-muted mt-2 mb-0">Progress: Cancelled</p>`;
                break;
            case 'error':
                startButtonDisabled = !bfWorkerAvailable; cancelButtonDisabled = true; applyBestButtonDisabled = !currentBestResultForMetric; showDetailsButtonDisabled = !currentBestResultForMetric;
                progressHTML = `<p class="mb-2 small text-danger">Error during optimization for cohort: <strong>${getCohortDisplayName(payload?.cohort || currentCohort)}</strong>.</p><div class="progress" style="height: 5px;"><div class="progress-bar bg-danger" role="progressbar" style="width: 0%;" aria-valuenow="0" aria-valuemin="0" aria-valuemax="100"></div></div><p class="small text-danger mt-2 mb-0">Error: ${payload?.message || 'Unknown error'}</p>`;
                break;
            default:
    ...
    function createAnalysisContextBannerHTML(context, patientCount) {
        ...
        let text = window.APP_CONFIG.UI_TEXTS.analysisContextBanner.text.replace('[CRITERIA_NAME]', criteriaName || 'the selected criteria').replace('[COHORT_NAME]', cohortName).replace('[COUNT]', count);
        return `<div class="alert alert-info small p-2 mt-3 text-center" role="alert" id="analysis-context-banner"><i class="fas fa-info-circle me-1"></i><strong>${window.APP_CONFIG.UI_TEXTS.analysisContextBanner.title}:</strong> ${text}</div>`;
    }
    ...
    function createAddedValueCardHTML(addedValueStats, t2SetName) {
        ...
        const fpData = addedValueStats?.t2FalsePositives, fnData = addedValueStats?.t2FalseNegatives;
        if (!fpData || !fnData) return '<p class="text-muted small p-2">Added value analysis data not available.</p>';
        const fpPerf = fpData.performanceAS, fnPerf = fnData.performanceAS;
        return `<div class="table-responsive"><table class="table table-sm table-striped small mb-0"><thead class="small"><tr><th>Subgroup (T2 Failure)</th><th>N</th><th>AS Specificity (95% CI)</th><th>AS Sensitivity (95% CI)</th></tr></thead><tbody><tr><td>T2 False Positives (T2+, N-)</td><td>${fpData.count}</td><td data-tippy-content="Of the ${fpData.count} patients incorrectly flagged by T2 criteria, the Avocado Sign correctly identified ${formatPercent(fpPerf?.spec?.value, 1)} as negative.">${fCI_p(fpPerf?.spec, 'spec')}</td><td>${na}</td></tr><tr><td>T2 False Negatives (T2-, N+)</td><td>${fnData.count}</td><td>${na}</td><td data-tippy-content="Of the ${fnData.count} patients missed by T2 criteria, the Avocado Sign correctly identified ${formatPercent(fnPerf?.sens?.value, 1)} as positive.">${fCI_p(fnPerf?.sens, 'sens')}</td></tr></tbody></table></div>`;
    }

    function createPowerAnalysisCardHTML(selectedStudyId) {
        const texts = window.APP_CONFIG.UI_TEXTS.insightsTab.powerAnalysis;
        const allStudySets = window.studyT2CriteriaManager.getAllStudyCriteriaSets(), allBfResults = window.bruteForceManager.getAllResults();
        let optionsHTML = '';
        const createOptions = (sets) => sets.map(set => `<option value="${set.id}" ${selectedStudyId === set.id ? 'selected' : ''}>${set.name || set.id}</option>`).join('');
        const groupedLitSets = allStudySets.reduce((acc, set) => { const group = set.group || 'Other Literature Criteria'; if (!acc[group]) acc[group] = []; acc[group].push(set); return acc; }, {});
        const bfOptionsHTML = Object.keys(allBfResults).map(cohortId => { const metric = window.APP_CONFIG.DEFAULT_SETTINGS.PUBLICATION_BRUTE_FORCE_METRIC; if (allBfResults[cohortId]?.[metric]) { const bfId = `bf_${cohortId}`; return `<option value="${bfId}" ${selectedStudyId === bfId ? 'selected' : ''}>Best Case T2 (${getCohortDisplayName(cohortId)})</option>`; } return ''; }).join('');
        ...
        groupOrder.forEach(groupName => { if (groupedLitSets[groupName]) optionsHTML += `<optgroup label="${groupName}">${createOptions(groupedLitSets[groupName])}</optgroup>`; });
        return `<div class="row g-4"><div class="col-md-5"><h6>Controls</h6><div class="mb-3"><label for="power-analysis-study-select" class="form-label small">${texts.selectLabel}</label><select class="form-select form-select-sm" id="power-analysis-study-select">${optionsHTML}</select></div><div class="mb-3"><label class="form-label small">${texts.modeLabel}</label><div class="form-check"><input class="form-check-input" type="radio" name="power-analysis-mode" id="power-mode-posthoc" value="posthoc" checked><label class="form-check-label small" for="power-mode-posthoc">${texts.postHocModeLabel}</label></div><div class="form-check"><input class="form-check-input" type="radio" name="power-analysis-mode" id="power-mode-samplesize" value="samplesize"><label class="form-check-label small" for="power-mode-samplesize">${texts.sampleSizeModeLabel}</label></div></div><div id="power-analysis-inputs"></div></div><div class="col-md-7 d-flex align-items-center justify-content-center bg-light rounded p-4"><div id="power-analysis-results" class="w-100"></div></div></div>`;
    }
    ...
    function createNodeCountAnalysisCardHTML(selectedLitSetId) {
        ...
        const groupOrder = ['ESGAR Criteria', 'Other Literature Criteria'];
        groupOrder.forEach(groupName => { if (groupedLitSets[groupName]) optionsHTML += `<optgroup label="${groupName}">${createOptions(groupedLitSets[groupName])}</optgroup>`; });
        return `<div class="row g-4"><div class="col-md-5"><h6>Controls</h6><div class="mb-3"><label for="node-count-lit-set-select" class="form-label small">${texts.selectLabel}</label><select class="form-select form-select-sm" id="node-count-lit-set-select">${optionsHTML}</select></div></div><div class="col-md-7"><div id="node-count-analysis-results" class="w-100"><p class="text-muted small">Select a criteria set to view aggregate lymph node counts.</p></div></div></div>`;
    }
    ...
    function createMismatchAnalysisCardHTML(mismatchData) {
        ...
        const { falseNegatives, discordance } = mismatchData, texts = window.APP_CONFIG.UI_TEXTS.insightsTab.mismatchAnalysis, na = window.APP_CONFIG.NA_PLACEHOLDER;
        const createDiscordanceCard = (title, count, description) => `<div class="card h-100 border text-center"><div class="card-body p-2 d-flex flex-column justify-content-center"><h6 class="card-subtitle mb-2 text-muted small">${title}</h6><h3 class="card-title text-primary">${count}</h3><p class="card-text small text-muted">${description}</p></div></div>`;
        const discordanceHTML = `<div class="row g-2 mb-4"><div class="col-md-3">${createDiscordanceCard(texts.concordantCorrect, discordance.bothCorrect.length, 'Both AS and T2 classified correctly.')}</div><div class="col-md-3">${createDiscordanceCard(texts.asSuperior, discordance.asCorrect.length, 'AS correct, T2 incorrect.')}</div><div class="col-md-3">${createDiscordanceCard(texts.t2Superior, discordance.t2Correct.length, 'T2 correct, AS incorrect.')}</div><div class="col-md-3">${createDiscordanceCard(texts.concordantIncorrect, discordance.bothWrong.length, 'Both AS and T2 classified incorrectly.')}</div></div>`;
        let fnRows = (falseNegatives && falseNegatives.length > 0) ? falseNegatives.map(p => `<tr><td>${p.id}</td><td>${p.age} / ${p.sex}</td><td>${getCohortDisplayName(p.therapy)}</td><td>${p.nStatus} (${p.countPathologyNodesPositive}/${p.countPathologyNodes})</td><td>${formatNumber(p.maxNodeSize, 1, na)} mm</td><td>${p.countT2Nodes}</td><td><small class="text-muted">${p.notes || ''}</small></td></tr>`).join('') : '<tr><td colspan="7" class="text-center text-muted">No AS-False Negatives found (AS correctly identified all N+ patients).</td></tr>';
        const fnTableHTML = `<h6 class="mb-2">${texts.fnAnalysis} (AS-/N+, n=${falseNegatives.length})</h6><div class="table-responsive"><table class="table table-sm table-striped small table-hover border"><thead class="table-light"><tr><th>ID</th><th>Age/Sex</th><th>Cohort</th><th>Pathology (Pos/Total)</th><th>Max T2 Size</th><th>T2 Nodes</th><th>Notes</th></tr></thead><tbody>${fnRows}</tbody></table></div>`;
        return `<div class="container-fluid p-0"><p class="small text-muted mb-3">${texts.description}</p>${discordanceHTML}${fnTableHTML}</div>`;
    }
